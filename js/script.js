/* alert("Entró"); */
const rows = 40;/* # de filas */
const cols = 40; /* # de columnas */
let board = []; /* Teblero. Array bidimensional */
...
function createBoard() {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < cols; j++) {
            ...
            cell.className = 'cell'; /* Le asigna la div la clase cell */
            cell.title = `(${i}, ${j})`;
            cell.onclick = () => toggleCell(i, j);
            gameBoard.appendChild(cell); /* Agrega el div de la celda como hijo de gamBoard */
            board[i][j] = cell;
        }
    }
}
// Agrega la clase live a la celda específica//
...
function updateBoard() {
    ...
    let changes = []; /* Almacena los cambios para aplicarlos después de calcular todos los estados */
...
function calculateBoardStatus(changes) {
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            let liveCount = countLiveNeighboard(i, j);/* Contamos las vecinas vivas */

            let isLive = board[i][j].classList.contains('live');


            if(isLive && (liveCount < 2 || liveCount > 3)) { /* Primera regla */
                //Muere por soledad o por sobrepoblación
                changes.push({i, j, live: false});
                console.log('Esta muerta');
            } else if (!isLive && liveCount === 3) { //Segunda Regla
                //Si está muerta y tiene 3 vecinas vivas, nace una nueva célula.
...
function countLiveNeighboard(row, col) {
    let count = 0;
